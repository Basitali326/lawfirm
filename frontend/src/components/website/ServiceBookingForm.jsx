...
function firmQuery() {
  const slug = process.env.NEXT_PUBLIC_STOREFRONT_FIRM_SLUG || process.env.NEXT_PUBLIC_FIRM_SLUG || "";
  return slug ? `?firm_slug=${encodeURIComponent(slug)}` : "";
}

export default function ServiceBookingForm({ service }) {
  const [date, setDate] = useState("");
  const [slots, setSlots] = useState([]);
  const [slot, setSlot] = useState("");
  const [form, setForm] = useState({ client_name: "", client_email: "", client_phone: "", message: "", appointment_type: service.supports_online ? "ONLINE" : "PHYSICAL" });
  ...
  async function loadSlots(nextDate) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/website/services/${service.slug}/slots/${query}${separator}date=${nextDate}`);
  ...
  async function book(event) {
    ...
    if (!slot) return setError("Select an available appointment time.");
    setSaving(true);
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/website/appointment-checkout/${firmQuery()}`, {
        method: "POST",
  ...
  return (
    <form onSubmit={book} className="h-fit space-y-5 rounded-2xl border border-[#d8ccb6] bg-white p-7 shadow-xl lg:sticky lg:top-28">
      ...
      <label className="block"><span className="mb-2 block text-sm font-semibold">Message (optional)</span><textarea rows={3} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} className="w-full rounded-xl border border-slate-300 px-4 py-3" /></label>
